...
class Similarity_Matrix_Recommender(object):
    ...
    def recommend(self, user_id, at=None, exclude_seen=True, filterTopPop = False, filterCustomItems = False):
        ...
        relevant_items_partition = (-scores).argpartition(at)[0:at]
    ...
    def recommendBatch(self, users_in_batch, at=None, exclude_seen=True, filterTopPop = False, filterCustomItems = False):
        ...
        ranking = np.zeros((scores_array.shape[0],at), dtype=np.int)
    ...
    def recommend_new_user(self, user_profile, at=None, exclude_seen=True):
        # compute the scores using the dot product
        if self.sparse_weights:
